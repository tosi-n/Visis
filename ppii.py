...
# search_dir = "/Volumes/Loopdisk/Visis/Gutenberg"
# fn = "/Volumes/Loopdisk/Visis/Gutenberg_out/corpus_1.txt"
...
error_rate = 0.6
reverse = True
model_path = '/Volumes/Loopdisk/Visis/model/seq2seq.h5'
hidden_size = 512
sample_mode = 'argmax'
data_path = '/Volumes/Loopdisk/Visis/Gutenberg_out'
books = ['corpus_1.txt', 'Abraham-Lincoln___The-Writings-of-Abraham-Lincoln-Volume-7-1863-1865.txt', 'Agatha-Christie___The-Mysterious-Affair-at-Styles.txt', 'Aldous-Huxley___Crome-Yellow.txt']



def deepSpell(sample, option):
    text  = read_text(data_path, books)
    vocab = tokenize(text)
    vocab = list(filter(None, set(vocab)))
    with open(sample, 'r') as trans_file:
        content = trans_file.read().split()
        test_sentence =' '.join(x for x in content)
    ...
    if option == 1:
        return ' '.join([token for token in decoded_tokens])
    else:
